...
export default function StudentsManagementPage() {
  ...
  const handleAddStudent = () => {
    if (!firestore || !newStudent.name || !newStudent.regId || !newStudent.departmentId) {
      toast({ variant: "destructive", title: "بيانات ناقصة", description: "يرجى ملء كافة الحقول الأساسية." });
      return;
    }
      ...
      <main className={cn(
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-muted-foreground font-bold">تحديث سجلات الطلاب الحقيقية لمطابقة بيانات الأرشفة</p>
          </div>
                    ...
                    <div className="space-y-2 text-right">
                      ...
                      <Select onValueChange={(v) => setNewStudent({...newStudent, departmentId: v})}>
                        ...
                        <SelectContent className="rounded-xl font-bold">
                          {loadingDepts ? (
                            <div className="p-2 text-center text-xs opacity-50">جاري تحميل الأقسام...</div>
          ...
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 p-6 bg-muted/20 rounded-2xl animate-slide-up">
              ...
              <div className="flex items-end">
                <Button 
                  variant="ghost" 
                  onClick={() => { setFilterDept("all"); setFilterLevel("all"); }}
