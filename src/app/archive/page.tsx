    ...
    <div className="min-h-screen bg-background">
      ...
      <main className={cn(
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 className="text-3xl font-black text-primary mb-1">الأرشيف المركزي</h1>
            <p className="text-muted-foreground font-bold">إدارة ومراجعة كافة الاختبارات المؤرشفة سحابياً</p>
          </div>
          ...
          <div className="flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border">
            <Button 
              ...
              className={cn("rounded-xl px-4 gap-2", view === 'grid' && "gradient-blue shadow-md")}
            >
            ...
            <Button 
              ...
              className={cn("rounded-xl px-4 gap-2", view === 'list' && "gradient-blue shadow-md")}
            >
          ...
          {showFilters && (
            <Card className="p-6 rounded-[2rem] shadow-lg border-none bg-white animate-slide-up grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-black text-primary mr-1">السنة الدراسية</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-black text-primary mr-1">الفصل الدراسي</label>
                  ...
                  {processedResults.map((item) => (
                    <Card key={item.id} className="group overflow-hidden border-none shadow-xl rounded-[2rem] bg-white hover:-translate-y-2 transition-all">
                      <div className="relative aspect-[3/4] bg-muted/30 overflow-hidden">
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end gap-3 backdrop-blur-[2px]">
                          ...
                          <Button 
                            ...
                            variant="outline" 
                            className="w-full rounded-xl bg-white/10 text-white border-white/20 font-black backdrop-blur-md hover:bg-white/20"
                          >
                      ...
                      <div className="p-6 text-right">
                        ...
                        <p className="text-sm text-secondary font-black mb-4 flex items-center justify-end gap-2">
                          {item.subjectName}
                          <BookOpen className="w-3 h-3" />
                        </p>
                        <div className="flex items-center justify-between text-[10px] text-muted-foreground border-t pt-4 font-bold">
                          <div className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {item.uploadedAt?.toDate ? item.uploadedAt.toDate().toLocaleDateString('ar-EG-u-nu-latn') : 'قيد الأرشفة'}</div>
                    ...
                    <table className="w-full text-right">
                      <thead>
                        <tr className="bg-muted/30 border-b">
                          ...
                          <th className="p-6 font-black text-primary">تاريخ الرفع</th>
                      ...
                      <tbody>
                        {processedResults.map((item) => (
                          <tr key={item.id} className="border-b hover:bg-muted/10 transition-colors group">
                            ...
                            <td className="p-6">
                              <Badge variant="outline" className="font-bold border-secondary text-secondary rounded-lg">{item.subjectName}</Badge>
                            </td>
                            <td className="p-6 text-xs font-bold text-muted-foreground">
                              {item.uploadedAt?.toDate ? item.uploadedAt.toDate().toLocaleDateString('ar-EG-u-nu-latn') : '---'}
                            </td>
          ...
          {viewingExam && (
            <div className="relative w-full h-[85vh] flex flex-col">
              <div className="absolute top-6 right-6 z-50">
                <Button variant="ghost" size="icon" onClick={() => setViewingExam(null)} className="rounded-full bg-black/10 hover:bg-black/20"><X className="w-6 h-6" /></Button>
              </div>
              <div className="flex-1 relative">
                <Image src={viewingExam.fileUrl || PlaceHolderImages[1].imageUrl} alt="Exam Full" fill className="object-contain" priority />
              </div>
              <div className="p-8 bg-white/80 backdrop-blur-md border-t flex items-center justify-between">
                ...
                <Button onClick={() => handleDownload(viewingExam)} className="rounded-2xl h-12 px-8 font-black gradient-blue shadow-lg gap-2">
