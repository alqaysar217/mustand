    ...
    <div className="min-h-screen bg-background">
      ...
      <main className={cn(
        ...
        <div className="max-w-md mx-auto mb-12">
          <Tabs value={mode} onValueChange={(v) => {
            ...
            <TabsList className="bg-white p-1 rounded-2xl h-14 shadow-lg border w-full">
              ...
              <TabsTrigger value="manual" className={cn("flex-1 rounded-xl font-black transition-all gap-2", mode === 'manual' ? "gradient-blue text-white shadow-md" : "text-muted-foreground")}><Keyboard className="w-4 h-4" />أرشفة يدوية (سريع)</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
          ...
          {[1, 2, 3, 4, 5].map((s) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn("absolute -bottom-10 whitespace-nowrap text-[11px] font-black transition-colors", step >= s ? "text-primary" : "text-muted-foreground")}>
                  {s === 1 && 'السياق'} {s === 2 && 'الرفع'} {s === 3 && 'المعاينة'} {s === 4 && 'التحليل'} {s === 5 && (mode === 'manual' ? 'التعريف' : 'التأكيد')}
                </span>
              </div>
            )
          })}
        ...
        <Card className="p-8 md:p-12 border-none shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-[40px] bg-white min-h-[520px] flex flex-col relative overflow-hidden">
          {loading && (
            <div className="absolute inset-0 bg-white/90 backdrop-blur-xl z-50 flex flex-col items-center justify-center gap-6 text-center animate-fade-in">
          ...
          {step === 1 && (
            <div className="space-y-10 animate-slide-up flex-1">
              <div className="flex items-center gap-4 border-b pb-6">
                <div className="p-3 bg-primary/5 rounded-2xl border border-primary/10 text-primary"><Info className="w-7 h-7" /></div>
          ...
          {step === 2 && (
            <div className="flex-1 flex flex-col items-center justify-center animate-slide-up">
              <div onClick={() => fileInputRef.current?.click()} className="w-full max-w-xl h-80 border-4 border-dashed border-primary/20 rounded-[50px] flex flex-col items-center justify-center gap-8 cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all group shadow-inner bg-muted/10">
                <div className="p-10 bg-white rounded-full group-hover:scale-110 transition-transform shadow-xl border border-primary/5"><FileUp className="w-20 h-20 text-primary" /></div>
                <div className="text-center px-10"><p className="text-3xl font-black text-primary mb-3">رفع أوراق الاختبار</p><p className="text-sm text-muted-foreground font-bold bg-white px-4 py-2 rounded-full border shadow-sm">المادة المختارة: {formData.subjectName}</p></div>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="animate-slide-up flex-1">
               <div className="flex items-center justify-between mb-10 border-b pb-6">
                 <div><h2 className="text-2xl font-black text-primary">معاينة صفحات الاختبار</h2><p className="text-muted-foreground font-bold text-sm">تأكد من وضوح كافة الصفحات قبل الحفظ</p></div>
                 <Button variant="outline" onClick={() => fileInputRef.current?.click()} className="rounded-2xl h-12 px-6 font-black text-secondary border-2 gap-2 hover:bg-secondary/5 transition-all"><Plus className="w-5 h-5" />إضافة صفحة</Button>
               </div>
               <div className="grid grid-cols-2 sm:grid-cols-4 gap-8">
                 {files.map((f, i) => (
                   <div key={i} className="relative group aspect-[3/4] rounded-3xl overflow-hidden border-4 border-white shadow-2xl transition-transform hover:-translate-y-2">
                     ...
                     <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm"><Button size="icon" variant="destructive" className="rounded-2xl w-12 h-12 shadow-xl" onClick={() => setFiles(files.filter((_, idx) => idx !== i))}><Trash2 className="w-5 h-5" /></Button></div>
          ...
          {step === 4 && mode === 'ai' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-12">
              <div className="relative"><div className="w-48 h-48 bg-secondary/5 rounded-full flex items-center justify-center shadow-inner overflow-hidden"><Scan className="w-24 h-24 text-secondary animate-pulse" /></div><div className="absolute -top-6 -right-6 w-16 h-16 bg-white rounded-3xl shadow-2xl flex items-center justify-center border-4 border-primary/5"><Sparkles className="w-8 h-8 text-primary animate-bounce" /></div></div>
              <div className="max-w-lg"><h2 className="text-4xl font-black text-primary mb-4">التحليل والمطابقة الذكية</h2><p className="text-muted-foreground font-bold text-lg leading-relaxed">سيقوم محرك الذكاء الاصطناعي بمطابقة رقم القيد المستخرج من الورقة مع سجلات الطلاب الرسمية.</p></div>
              <Button onClick={handleOCR} disabled={loading} className="h-20 px-16 rounded-[2rem] text-2xl font-black gradient-blue shadow-lg hover:scale-105 transition-all group">{loading ? <Loader2 className="w-8 h-8 animate-spin ml-4" /> : <Scan className="w-8 h-8 ml-4" />}بدء المطابقة الآن</Button>
            </div>
          )}

          {step === 5 && (
            <div className="animate-slide-up flex-1 space-y-10">
               <div className={cn("flex items-center gap-6 p-8 rounded-[40px] border-2 shadow-sm transition-all", extractedData.found ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-200")}>
                  <div className={cn("w-16 h-16 rounded-3xl flex items-center justify-center shadow-lg", extractedData.found ? "bg-green-500 text-white" : "bg-orange-500 text-white")}>{isSearching ? <Loader2 className="w-9 h-9 animate-spin" /> : extractedData.found ? <UserCheck className="w-9 h-9" /> : <AlertCircle className="w-9 h-9" />}</div>
                  <div className="text-right flex-1"><h2 className={cn("text-2xl font-black", extractedData.found ? "text-green-800" : "text-orange-800")}>{isSearching ? "جاري المطابقة..." : extractedData.found ? "تم التعرف على الطالب" : "تنبيه: الطالب غير مسجل"}</h2><p className="text-base font-bold opacity-70">{isSearching ? "يتم الآن جلب بيانات الطالب من السجلات الرسمية..." : extractedData.found ? `تمت مطابقة رقم القيد (${extractedData.id}) مع سجلات الطالب ${extractedData.name}.` : "رقم القيد المدخل لا يطابق أي طالب مسجل. يرجى التأكد من الرقم."}</p></div>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3 text-right">
                    ...
                    <div className="relative group"><input value={extractedData.id} onChange={(e) => handleRegIdChange(e.target.value)} className={cn("w-full h-16 pr-14 pl-6 rounded-3xl border-2 bg-white font-black text-2xl text-right outline-none transition-all shadow-inner", extractedData.found ? "border-green-300 focus:border-green-500" : "border-muted focus:border-primary")} placeholder="أدخل رقم القيد..." /><div className="absolute right-4 top-1/2 -translate-y-1/2">{isSearching ? <Loader2 className="w-6 h-6 animate-spin text-primary" /> : extractedData.found ? <CheckCircle2 className="w-6 h-6 text-green-500" /> : <Search className="w-6 h-6 text-muted-foreground" />}</div></div>
                  </div>
                  <div className="space-y-3 text-right">
                    ...
                    <div className="relative"><input value={extractedData.name} readOnly={extractedData.found} onChange={(e) => setExtractedData({...extractedData, name: e.target.value})} className={cn("w-full h-16 pr-14 pl-6 rounded-3xl border-2 bg-muted/10 font-black text-lg text-right outline-none transition-all shadow-inner", extractedData.found ? "bg-green-50/50 border-green-200 text-green-800" : "bg-white border-muted focus:border-primary")} placeholder={extractedData.found ? "" : "أدخل الاسم يدوياً..."} /><div className="absolute right-4 top-1/2 -translate-y-1/2">{extractedData.found ? <UserCheck className="w-6 h-6 text-green-500" /> : <User className="w-6 h-6 text-muted-foreground" />}</div></div>
                  </div>
               </div>
            </div>
          )}
          ...
          <div className="mt-auto pt-10 flex items-center justify-between border-t-2 border-muted/50">
            <Button variant="outline" onClick={prevStep} disabled={step === 1 || loading} className="h-16 px-10 rounded-[1.5rem] border-2 border-muted font-black gap-4 shadow-sm hover:bg-muted/10 transition-all flex items-center">السابق<ChevronRight className="w-6 h-6" /></Button>
            <div className="flex gap-4">
              {(step < 5 && !(step === 3 && mode === 'manual')) ? (
                <Button onClick={nextStep} disabled={(step === 2 && files.length === 0) || (step === 1 && (!formData.subjectId || !formData.level)) || (step === 3 && files.length === 0)} className="h-16 px-16 rounded-[1.5rem] font-black gap-4 gradient-blue shadow-lg hover:scale-105 transition-all flex items-center"><ChevronLeft className="w-6 h-6" />التالي</Button>
              ) : step === 3 && mode === 'manual' ? (
                <Button onClick={nextStep} className="h-16 px-16 rounded-[1.5rem] font-black gap-4 gradient-blue shadow-lg hover:scale-105 transition-all flex items-center"><ChevronLeft className="w-6 h-6" />تحديد الطالب</Button>
              ) : step === 5 ? (
                <Button onClick={handleSaveToArchive} disabled={loading || !extractedData.id || isSearching} className="h-16 px-16 rounded-[1.5rem] font-black gap-4 bg-green-600 text-white shadow-lg hover:bg-green-700 hover:scale-105 transition-all"><CloudUpload className="w-6 h-6" />حفظ نهائي في الأرشيف</Button>
              ) : null}
            </div>
          </div>
        </Card>
