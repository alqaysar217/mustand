...
export default function UploadPage() {
  ...
  const saveManualArchive = async () => {
    ...
    try {
      ...
      toast({ title: "تمت الأرشفة اليدوية بنجاح" });
      setDocs([]);
      setManualStudent(null);
      setManualId("");
      setStep(1);
  ...
  const processSingleDocAI = async (doc: ArchiveDocument): Promise<AIResult> => {
    try {
      const responseData = await extractExamDetails({ 
        examImageDataUri: doc.coverImage,
        selectedSubject: context.subjectName,
        selectedDept: context.deptName,
        selectedLevel: context.level,
        selectedTerm: context.term
      });
      ...
      // مطابقة ذكية للمواد: ترجمة ومعنى (Semantic Matching)
      const contextSub = normalize(context.subjectName);
      ...
      const isSemanticMatch = (contextSub.includes(aiSub) || aiSub.includes(contextSub)) || 
                             ...
                             (contextSub === "datamining" && aiSub === "تنقيبالبيانات");

      const subjectMatch = isSemanticMatch;
      
      const contextDept = normalize(context.deptName);
      const aiDept = normalize(responseData.departmentName);
      const deptMatch = aiDept.includes(contextDept) || contextDept.includes(aiDept);
      ...
      const collegeMatch = !context.collegeName || normalize(responseData.collegeName).includes(normalize(context.collegeName)) || 
                          normalize(context.collegeName).includes(normalize(responseData.collegeName));

      // فشل المطابقة إذا كانت المادة غير صحيحة
      const isMismatch = !subjectMatch;
  ...
  const startAIAnalysis = async () => {
    ...
    setLoading(true);
    setLoadingText("جاري التحليل والمطابقة الخماسية...");
  ...
  const saveBatchAI = async () => {
    const validResults = aiResults.filter(r => r.isVerified && r.sizeKB < 1000 && r.status === 'success');
    if (!firestore || validResults.length === 0) return;
    ...
    try {
      ...
      toast({ title: `تم اعتماد وأرشفة ${validResults.length} ملف.` });
      setAiResults(aiResults.filter(r => r.status !== 'success'));
      if (aiResults.filter(r => r.status !== 'success').length === 0) { setDocs([]); setStep(1); }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-right" dir="rtl">
      <Sidebar />
      <Navbar />
      <main className={cn("transition-all duration-300 p-4 md:p-10 animate-fade-in max-w-7xl mx-auto", isOpen ? "mr-0 md:mr-64" : "mr-0")}>
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8 md:mb-12">
          <div className="text-center md:text-right">
            <h1 className="text-3xl md:text-4xl font-black text-primary mb-2">بوابة الأرشفة الرقمية</h1>
            <p className="text-muted-foreground font-bold text-sm">نظام المطابقة الخماسية المدعوم بالذكاء الاصطناعي</p>
          </div>
          <Tabs value={activeMode} onValueChange={(v: any) => { setActiveMode(v); setStep(1); setDocs([]); setAiResults([]); setManualId(""); setManualStudent(null); }} className="w-full md:w-[400px]">
            <TabsList className="grid w-full grid-cols-2 h-14 md:h-16 bg-white rounded-2xl p-1.5 shadow-xl border overflow-hidden">
              <TabsTrigger 
                value="manual" 
                className={cn(
                  "rounded-xl font-black text-sm transition-all",
                  activeMode === 'manual' ? "gradient-blue text-white shadow-lg" : "text-muted-foreground hover:bg-muted/50"
                )}
              >
                أرشفة يدوية
              </TabsTrigger>
              <TabsTrigger 
                ...
                className={cn(
                  "rounded-xl font-black text-sm transition-all",
                  activeMode === 'ai' ? "gradient-blue text-white shadow-lg" : "text-muted-foreground hover:bg-muted/50"
                )}
              >
                أرشفة ذكية
              </TabsTrigger>
        ...
        {step === 1 && (
          <Card className="p-6 md:p-12 border-none shadow-2xl rounded-[2rem] bg-white animate-slide-up border-t-8 border-primary">
            <div className="flex items-center gap-4 mb-8 border-b pb-6">
              ...
              <div className="text-right"><h2 className="text-2xl font-black text-primary">تحديد سياق المادة</h2><p className="text-sm font-bold text-muted-foreground">تحديد البيانات للمطابقة التلقائية</p></div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-10">
              ...
              <div className="space-y-2">
                ...
                <select value={context.collegeName} onChange={(e) => setContext({...context, collegeName: e.target.value})} className="w-full h-12 px-4 rounded-xl border-2 bg-muted/5 font-black text-sm outline-none focus:border-primary text-right">
              ...
              <div className="space-y-2">
                ...
                <select value={context.level} onChange={(e) => setContext({...context, level: e.target.value})} className="w-full h-12 px-4 rounded-xl border-2 bg-muted/5 font-black text-sm outline-none focus:border-primary text-right">
                  ...
                  {["المستوى الأول", "المستوى الثاني", "المستوى الثالث", "المستوى الرابع"].map(l => (<option key={l} value={l}>{l}</option>))}
                </select>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <select value={context.subjectId} onChange={(e) => {
            ...
            <Button onClick={() => setStep(2)} disabled={!context.subjectId} className="w-full md:w-auto md:px-20 h-16 rounded-2xl text-xl font-black gradient-blue shadow-2xl gap-4 text-white mx-auto flex">متابعة لرفع الدفاتر <ArrowLeft className="w-6 h-6" /></Button>
          </Card>
        )}
          ...
          <div className="space-y-6 animate-slide-up pb-20">
            <div className="bg-white p-6 rounded-2xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 border-r-8 border-secondary">
               <div className="flex items-center gap-5">
                  ...
                  <div className="text-right"><h3 className="text-2xl font-black text-primary">{context.subjectName}</h3><p className="text-sm font-bold text-muted-foreground">{context.deptName} • {context.level} • {context.year}</p></div>
               </div>
               <Button variant="outline" size="sm" onClick={() => setStep(1)} className="rounded-xl font-black h-12 px-6 border-2 gap-2"><RefreshCcw className="w-4 h-4" /> تغيير السياق</Button>
            </div>

            <Card className="p-8 border-none shadow-2xl rounded-[2rem] bg-white">
              <div className="flex items-center justify-between mb-8">
                  <h2 className="text-2xl font-black text-primary flex items-center gap-3"><FileText className="w-7 h-7 text-secondary" />الملفات المختارة <Badge variant="secondary" className="bg-muted px-2.5 rounded-lg text-primary">{docs.length}</Badge></h2>
                  <Button variant="ghost" onClick={() => fileInputRef.current?.click()} className="text-secondary font-black hover:bg-secondary/5 rounded-xl text-sm"><UserPlus className="w-5 h-5 ml-2" /> إضافة المزيد</Button>
               </div>
              <div onClick={() => docs.length === 0 && fileInputRef.current?.click()} className={cn("w-full min-h-[220px] rounded-[2rem] flex flex-col items-center justify-center gap-4 transition-all", docs.length === 0 ? "border-4 border-dashed border-muted cursor-pointer hover:border-primary hover:bg-primary/5" : "bg-muted/10 p-8")}>
                {docs.length > 0 ? (
                ...
                ) : (<div className="flex flex-col items-center gap-2 text-center"><div className="w-20 h-20 bg-primary/10 rounded-2xl flex items-center justify-center text-primary mb-2"><FileUp className="w-10 h-10" /></div><p className="text-xl font-black text-primary">اسحب الدفاتر هنا</p><p className="text-sm font-bold text-muted-foreground">تأكد أن حجم الملف لا يتجاوز 1MB.</p></div>)}
              </div>
              ...
              {docs.length > 0 && activeMode === 'ai' && aiResults.length === 0 && (<Button onClick={startAIAnalysis} className="w-full mt-10 rounded-2xl font-black gradient-blue shadow-2xl text-white h-20 text-2xl hover:scale-[1.01] transition-all"><Scan className="w-8 h-8 ml-4" /> تحليل الأغلفة والمطابقة</Button>)}
            </Card>

            {aiResults.length > 0 && (
              <div className="space-y-8 animate-slide-up pb-20">
                <div className="bg-white px-10 py-6 rounded-[2.5rem] shadow-xl border-r-8 border-green-500 flex items-center justify-between"><div className="text-right"><h2 className="text-2xl font-black text-primary">نتائج التدقيق الذكي</h2><p className="text-muted-foreground font-bold">تحليل ومطابقة {aiResults.length} دفاتر مع بيانات المادة</p></div></div>
                <div className="grid grid-cols-1 gap-6">
                   {aiResults.map((res, i) => (
                     <Card key={i} className={cn("p-6 rounded-[2.5rem] border-4 flex flex-col md:flex-row items-center gap-8 bg-white shadow-xl relative overflow-hidden transition-all", res.status === 'success' ? "border-green-400" : res.status === 'error' ? "border-red-500" : res.status === 'mismatch' ? "border-orange-400" : "border-red-400")}>
                        <div className="w-32 h-44 relative rounded-xl overflow-hidden shadow-lg shrink-0 border-2 border-white"><Image src={res.coverImage} alt="Cover" fill className="object-cover" unoptimized /></div>
                        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full text-right">
                           <div className="space-y-1.5">
                              <Label className="text-[10px] font-black text-muted-foreground flex items-center gap-1">بيانات الطالب</Label>
                              <div className={cn("h-10 rounded-xl px-3 bg-muted/20 flex items-center font-black text-xs truncate", !res.isVerified && "text-red-500")}>{res.studentName}</div>
                           ...
                           {res.status === 'error' ? (
                             <div className="col-span-2 flex flex-col justify-center"><div className="flex items-center gap-2 text-red-600 font-bold text-sm mb-1"><AlertCircle className="w-5 h-5" />خطأ في التحليل</div><p className="text-xs text-muted-foreground bg-red-50 p-2 rounded-lg border border-red-100">{res.errorMessage}</p></div>
                           ) : (
                             <>
                               <div className="space-y-1.5">
                                  <Label className="text-[10px] font-black text-muted-foreground flex items-center gap-1">المادة المستخرجة</Label>
                                  <div className={cn("h-10 rounded-xl px-3 flex items-center font-bold text-xs truncate border-2", res.matchScore.subject ? "bg-green-50 border-green-200 text-green-700" : "bg-orange-50 border-orange-300 text-orange-800")}>
                                    {res.extractedSubject || "لم يتم التعرف"}
                                    {res.matchScore.subject ? <Check className="w-3 h-3 mr-auto" /> : <X className="w-3 h-3 mr-auto" />}
                                  </div>
                                  {!res.matchScore.subject && <p className="text-[9px] text-orange-600 font-bold mt-1">المادة في الورقة تختلف عن المختارة!</p>}
                               </div>
                               <div className="space-y-1.5">
                                  <Label className="text-[10px] font-black text-muted-foreground flex items-center gap-1">التخصص والمستوى</Label>
                                  <div className={cn("h-10 rounded-xl px-3 flex items-center font-bold text-[10px] truncate border-2", res.matchScore.dept ? "bg-green-50 border-green-200 text-green-700" : "bg-orange-50 border-orange-300 text-orange-800")}>
                                    {res.extractedDept || "تخصص غير معروف"}
                                    {res.matchScore.dept ? <Check className="w-3 h-3 mr-auto" /> : <X className="w-3 h-3 mr-auto" />}
                                  </div>
                                  <div className="flex gap-1.5 mt-1">
                                    <Badge variant="outline" className={cn("text-[8px] font-black h-6", res.matchScore.level ? "text-green-600 border-green-200" : "text-orange-600 border-orange-200")}>{res.extractedLevel || "المستوى ؟"}</Badge>
                                    <Badge variant="outline" className={cn("text-[8px] font-black h-6", res.matchScore.term ? "text-green-600 border-green-200" : "text-orange-600 border-orange-200")}>{res.extractedTerm || "الترم ؟"}</Badge>
                                  </div>
                               </div>
                             </>
                           )}
                ...
                <div className="flex gap-4">
                  <Button onClick={saveBatchAI} disabled={aiResults.filter(r => r.isVerified && r.sizeKB < 1000 && r.status === 'success').length === 0} className="flex-1 h-24 rounded-[3rem] text-3xl font-black gradient-blue shadow-2xl text-white gap-6">اعتماد وأرشفة المتطابق ({aiResults.filter(r => r.isVerified && r.sizeKB < 1000 && r.status === 'success').length})</Button>
                  <Button variant="outline" onClick={() => setAiResults([])} className="h-24 px-10 rounded-[3rem] border-4 font-black text-xl gap-3"><RotateCcw className="w-8 h-8" /> إعادة التحليل</Button>
                </div>
              </div>
            )}
            ...
            {activeMode === 'manual' && docs.length > 0 && aiResults.length === 0 && (
              <Card className="p-10 border-none shadow-2xl rounded-[2rem] bg-white animate-slide-up border-b-8 border-green-500">
                <div className="flex items-center justify-between mb-8"><h2 className="text-2xl font-black text-primary flex items-center gap-3"><CheckCircle2 className="w-7 h-7 text-green-600" /> التحقق اليدوي</h2></div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <div className="space-y-2"><Label className="font-black text-primary text-sm mr-1">رقم القيد</Label><div className="flex gap-2"><Input value={manualId} onChange={(e) => setManualId(e.target.value)} placeholder="0000" className="h-14 rounded-xl border-2 font-black text-xl text-center" onKeyDown={(e) => e.key === 'Enter' && identifyManualStudent(manualId)} /><Button onClick={() => identifyManualStudent(manualId)} className="h-14 w-14 rounded-xl gradient-blue text-white shrink-0"><Search className="w-6 h-6" /></Button></div></div>
                   <div className="space-y-2"><Label className="font-black text-primary text-sm mr-1">الاسم</Label><div className={cn("h-14 bg-muted/20 border-2 border-transparent rounded-xl px-4 flex items-center font-black text-lg", manualStudent ? "text-primary bg-primary/5" : "text-muted-foreground")}>{manualStudent?.name || "---"}</div></div>
                   <div className="space-y-2"><Label className="font-black text-primary text-sm mr-1">الحجم</Label><div className={cn("h-14 bg-muted/20 border-2 border-transparent rounded-xl px-4 flex items-center font-black text-lg", docs[0].sizeKB > 1000 ? "text-red-600" : "text-green-600")}>{Math.round(docs[0].sizeKB/10.24)/100} MB</div></div>
                </div>
                {manualStudent && (<Button onClick={saveManualArchive} disabled={docs[0].sizeKB > 1000} className="w-full mt-12 h-20 rounded-[2.5rem] text-2xl font-black bg-green-600 hover:bg-green-700 text-white">اعتماد وأرشفة الدفتر</Button>)}
              </Card>
            )}
          </div>
        )}
