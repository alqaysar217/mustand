...
export default function UploadPage() {
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return (allSubjects as any[]).filter(s => {
      ...
      const matchLevel = s.level === context.level || s.level?.includes(context.level);
  ...
  const identifyStudent = async (regId: string) => {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        toast({ variant: "destructive", title: "الطالب غير موجود" });
      }
    } catch (e) {
  ...
  const startAIAnalysis = async () => {
    ...
    for (const file of files) {
      try {
        ...
        if (!response.ok) throw new Error('فشل استجابة الخادم');
  ...
  const saveBatchAI = async () => {
    ...
    try {
      ...
      toast({ title: `تم حفظ ${aiResults.length} ورقة بنجاح` });
      setFiles([]);
      setAiResults([]);
      setStep(1); 
    } catch (e) {
      ...
      <main className={cn(
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
          <div className="text-right">
            <h1 className="text-4xl font-black text-primary mb-2">الأرشفة الذكية</h1>
            <p className="text-muted-foreground font-bold text-lg">نظام التوثيق الرقمي للاختبارات</p>
          </div>
        ...
        {step === 1 && (
          <Card className="p-8 md:p-12 border-none shadow-2xl rounded-[2.5rem] bg-white animate-slide-up">
            <div className="flex items-center gap-4 mb-10 border-b pb-6 justify-start">
              ...
              <div className="text-right">
                <h2 className="text-2xl font-black text-primary">تثبيت السياق الأكاديمي</h2>
                <p className="text-muted-foreground font-bold">اختر المادة والقسم لبدء الأرشفة المتتابعة</p>
              </div>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-8 animate-slide-up">
            ...
            <div className="space-y-8">
              ...
              <Card className="p-8 border-none shadow-2xl rounded-[2.5rem] bg-white text-center">
                ...
                >
                  {files.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 p-6 w-full">
                      {files.map((f, i) => (
                        <div key={i} className="relative aspect-[3/4] rounded-xl overflow-hidden shadow-md border-2 border-white">
                ...
                {files.length > 0 && (
                  <div className="flex justify-center gap-4 mt-6">
                    <Button variant="destructive" onClick={() => { setFiles([]); setAiResults([]); }} className="rounded-xl font-black gap-2"><Trash2 className="w-4 h-4" /> مسح الصور</Button>
                    {activeMode === 'ai' && aiResults.length === 0 && (
                      <Button onClick={startAIAnalysis} className="rounded-xl font-black gradient-blue shadow-lg px-10 text-white gap-2 h-12">
                        <Scan className="w-5 h-5" /> بدء التحليل الذكي الفوري
                      </Button>
                    )}
                  </div>
                )}
              ...
              {/* منطقة المراجعة والتحقق - عرض كامل بالأسفل */}
              {activeMode === 'manual' ? (
                files.length > 0 && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                       <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Input value={manualId} onChange={(e) => setManualId(e.target.value)} placeholder="أدخل رقم القيد..." className="h-14 rounded-xl border-2 font-black text-xl text-center" onKeyDown={(e) => e.key === 'Enter' && identifyStudent(manualId)} />
                    ...
                    {manualStudent && (
                      <div className="mt-10">
                        <Button onClick={saveManualArchive} className="w-full h-16 rounded-2xl text-xl font-black bg-green-600 hover:bg-green-700 shadow-xl text-white gap-3">
                          <CloudUpload className="w-6 h-6" /> إتمام الأرشفة والحفظ
                        </Button>
                      </div>
                    )}
                  </Card>
                ...
                aiResults.length > 0 && (
                  <Card className="p-8 md:p-10 border-none shadow-2xl rounded-[2.5rem] bg-white space-y-8 animate-slide-up">
                    <div className="flex items-center justify-between border-b pb-6">
                       ...
                       <div className="bg-primary/5 text-primary px-6 py-2 rounded-xl font-black">{aiResults.length} مستند</div>
                    </div>

                    <div className="grid grid-cols-1 gap-6">
                       {aiResults.map((res, i) => (
                         <div key={i} className={cn("p-6 rounded-[2rem] border-2 flex flex-col md:flex-row items-center gap-8", res.isVerified ? "bg-green-50/30 border-green-100" : "bg-red-50/30 border-red-100")}>
                            <div className="w-24 h-32 relative rounded-xl overflow-hidden shadow-md shrink-0"><Image src={res.fileData} alt="Exam" fill className="object-cover" /></div>
                            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                               <div className="space-y-1">
                                  <Label className="text-[10px] font-black text-muted-foreground mr-1">اسم الطالب المستخرج</Label>
                               ...
                               <div className="space-y-1">
                                  ...
                                  <Input value={res.studentRegistrationId} onChange={(e) => handleUpdateAiResult(i, 'studentRegistrationId', e.target.value)} className="h-12 rounded-xl font-black text-lg bg-white" />
                               </div>
                               <div className="flex items-center justify-center md:justify-end gap-3">
                                  {res.isVerified ? (
                                    <div className="bg-green-500 text-white px-4 py-2 rounded-lg font-black text-xs flex items-center gap-2 shadow-sm"><CheckCircle2 className="w-4 h-4" /> مطابق</div>
                                  ) : (
                                    <div className="bg-red-500 text-white px-4 py-2 rounded-lg font-black text-xs flex items-center gap-2 shadow-sm"><XCircle className="w-4 h-4" /> غير مسجل</div>
                                  )}
                                  <Button size="icon" variant="ghost" onClick={() => setAiResults(prev => prev.filter((_, idx) => idx !== i))} className="text-destructive hover:bg-destructive/10 rounded-xl"><Trash2 className="w-5 h-5" /></Button>
                               </div>
                            </div>
                         </div>
                       ))}
                    ...
                    <div className="pt-8 border-t flex flex-col md:flex-row gap-4">
                       ...
                       <Button variant="outline" onClick={() => setAiResults([])} className="h-16 px-10 rounded-2xl font-black border-2">إلغاء وإعادة المحاولة</Button>
                    </div>
                  </Card>
                )
              )}
